...
class Res2NetFeatureBackbone(BaseFeatureBackbone):
    """具有四级特征的 Res2Net 自底向上主干网络。"""

    def __init__(self, model_name: str = "res2net50_26w_4s", pretrained: bool = False, pretrained_path: str = ""):
        import timm
...
def build_res2net50_fpn_backbone(cfg, input_shape=None):
    """构建 Res2Net-50-26w-4s 特征金字塔主干网络。"""
    pretrained = getattr(cfg.MODEL, "PRETRAINED", False)
    bottom_up = Res2NetFeatureBackbone(model_name="res2net50_26w_4s", pretrained=pretrained, pretrained_path=str(getattr(cfg.MODEL, "PRETRAINED_PATH", "")))
